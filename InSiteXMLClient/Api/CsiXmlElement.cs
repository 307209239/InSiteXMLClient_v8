...
namespace Camstar.XMLClient.API
{
    ...
    {
        private ICsiDocument mDocument = null;
        ...
        {
            ICsiXmlElement csiXmlElement = null;
        ...
        public ICsiXmlElement[] GetChildrenByName(string name) => CsiXmlHelper.GetChildrenByName(this.GetOwnerDocument(), this.GetDomElement(), name);

        public virtual ICsiXmlElement[] GetAllChildren() => this.GetAllChildren(false);

        protected internal XmlElement GetDomElement() => this.mDOMElement;

        protected internal ICsiXmlElement[] GetAllChildren(bool bDoNotIncludeTagsWith__)
        {
            ...
            for (XmlNode xmlNode = this.mDOMElement.FirstChild; xmlNode != null; xmlNode = xmlNode.NextSibling)
            {
                if (xmlNode.NodeType == XmlNodeType.Element && (!bDoNotIncludeTagsWith__ || !xmlNode.Name.StartsWith("__")))
                    arrayList.Add(CsiXmlHelper.CreateCsiElement(this.GetOwnerDocument(), (XmlElement)xmlNode));
            }
        ...
        protected ICsiXmlElement ClearXmlElementChildByName(string parent) => this.ClearXmlElementChildByName(parent, null);
        ...
        private List<XmlNode> GetImmediateChildren(XmlElement element)
        {
            var arrayList = new List<XmlNode>();
        ...
        {
            var immediateChildren = this.GetImmediateChildren(parent);
            int length = tag.IndexOf('[');
            if (length != -1)
            {
                tag.Substring(0, length);
                int index = int.Parse(tag.Substring(length + 1, tag.IndexOf(']') - (length + 1)));
                if (tag.Equals(immediateChildren[index].Name) && immediateChildren[index] is XmlElement)
