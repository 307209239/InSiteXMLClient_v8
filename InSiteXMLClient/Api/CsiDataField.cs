...
namespace Camstar.XMLClient.API
{
    internal class CsiDataField : CsiField, ICsiDataField, ICsiField, ICsiXmlElement
    {
        ...
        {
            this.mData = this.GetDomElement().OwnerDocument.CreateCDataSection("");
        ...
        private CsiDataField(
          ...
          : base(doc, name, parent)
        {
            this.mData = this.GetDomElement().OwnerDocument.CreateCDataSection(val);
            this.GetDomElement().AppendChild(this.mData);
        }
        ...
        public void SetFormattedValue(string val, DataFormats format)
        {
            ...
            catch (Exception ex)
            {
                ...
                throw new CsiClientException(-1L, "Can not convert '" + val + "' with format '" + format + "'. " + ex.Message, src);
            }
            if (val1 == null)
                return;
            this.SetValue(val1);
