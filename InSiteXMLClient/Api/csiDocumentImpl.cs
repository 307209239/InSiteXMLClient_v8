...
namespace Camstar.XMLClient.API
{
  internal class CsiDocument : ICsiDocument
  {
    ...
    public ICsiService CreateService(string serviceType) => !StringUtil.IsEmptyString(serviceType) ? (ICsiService) new CsiService((ICsiDocument) this, serviceType, this.mRootElement) : throw new CsiClientException(-1L, "Can not create Service with an empty name", this.GetType().FullName + ".createService()");
    ...
    public virtual void SetAlwaysReturnSelectionValues(bool val)
    {
      ...
      ICsiXmlElement parentElement2 = parentElement1.FindChildByName("__processingDefaults") ?? (ICsiXmlElement) new CsiXmlElement((ICsiDocument) this, "__processingDefaults", parentElement1);
      ICsiXmlElement childByName2 = parentElement2.FindChildByName("__alwaysGetSelectionValues");
      if (childByName2 == null)
      {
          ICsiXmlElement csiXmlElement = (ICsiXmlElement) new CsiXmlElement((ICsiDocument) this, "__alwaysGetSelectionValues", parentElement2);
      }
      else
    ...
    public virtual ICsiService[] GetServices()
    {
      ...
      for (int i = 0; i < elementsByTagName.Count; ++i)
      {
        ...
        csiServiceArray[i] = (ICsiService) new CsiService((ICsiDocument) this, xmlNode as XmlElement);
      }
    ...
    public virtual Array GetExceptions() => this.mExceptions.Count == 0 && !this.CheckErrors() ? (Array) null : (Array) this.mExceptions.ToArray();
    ...
    public ICsiRequestData RequestData()
    {
      if (!(this.GetRootElement().FindChildByName("__requestData") is CsiRequestData csiRequestDataImpl))
        csiRequestDataImpl = new CsiRequestData((ICsiDocument) this, this.GetRootElement());
      return (ICsiRequestData) csiRequestDataImpl;
    }

    public ICsiQuery CreateQuery() => (ICsiQuery) new CsiQuery((ICsiDocument) this, this.mRootElement);
    ...
    public virtual ICsiQuery[] GetQueries()
    {
      ...
      {
        XmlNode xmlNode = elementsByTagName[i];
        csiQueryArray[i] = (ICsiQuery) new CsiQuery((ICsiDocument) this, xmlNode as XmlElement);
      }
