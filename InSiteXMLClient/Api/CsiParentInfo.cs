...
namespace Camstar.XMLClient.API
{
  internal class CsiParentInfo : CsiXmlElement, ICsiParentInfo, ICsiXmlElement
  {
    ...
    private string GetRevision() => this.FindChildByName("__rev") is CsiXmlElement childByName ? CsiXmlHelper.GetFirstTextNodeValue(childByName) : (string) null;
    ...
    private string GetLevel() => this.FindChildByName("__level") is CsiXmlElement level && level.FindChildByName("__name") is CsiXmlElement childByName ? CsiXmlHelper.GetFirstTextNodeValue(childByName) : (string) null;
  }
}
