...
namespace Camstar.Util
{
  public class DateUtil
  {
    ...
    public static string DateTimeFormat => string.Format("{0} {1}", (object) DateUtil.DateFormat, (object) DateUtil.TimeFormat);

    public static string DateTimeFormatPHP => string.Format("{0} {1}", (object) DateUtil.DateFormat.ToLower().Replace("yyyy", "Y").Replace("yy", "y").Replace("d", "j").Replace("jj", "d").Replace("m", "n").Replace("nn", "m"), (object) DateUtil.TimeFormat.ToLower().Replace("hh", "H").Replace("mm", "i").Replace("ss", "s").Replace("tt", "A").Replace("fff", "u"));

    public static string ApplyUserLocaleSettings(
      string message,
      ...
      string outFormat)
    {
      ...
      {
        MatchCollection source = new Regex(string.Format("\\b({0})\\b", (object) regExpr)).Matches(message);
        if (source.Count > 0)
        {
